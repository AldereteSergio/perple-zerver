...
export class BrowserManager implements IBrowserManager {
  ...
  public searchInputSelector = '[role="textbox"]';
  ...
  private determineRecoveryLevel(error?: Error): number {
    ...
    if (errorMessage.includes("detached") || errorMessage.includes("crashed") || errorMessage.includes("disconnected") || errorMessage.includes("protocol error")) {
      return 3;
    }
    if (errorMessage.includes("navigation") || errorMessage.includes("timeout") || errorMessage.includes("net::err")) {
      return 2;
    }
    return 1;
  }
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      logError("BrowserManager initialization failed:", { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
  ...
  async waitForSearchInput(): Promise<string | null> {
    const ctx = this.getPuppeteerContext();
    return await waitForSearchInput(ctx);
  }
  ...
  async cleanup(): Promise<void> {
    try {
      ...
      logError("Error during BrowserManager cleanup:", { error: error instanceof Error ? error.message : String(error) });
    }
  }
